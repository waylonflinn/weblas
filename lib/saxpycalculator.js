...
void main(void) {                                                        \n\
	...
	vec4 sum_v = (a * texture2D( X, vec2(col, row))) + texture2D( Y, vec2(col, row));                                    \n\
	float sum; \n\
	int type = int(mod(floor(col * float(N)), 4.0 )); \n\
	if (type == 0) {\n\
		sum = sum_v.r;\n\
	} \n\
	if(type == 1) {\n\
...
SAXPYCalculator.prototype.calculate = function(N, a, X, Y){
	...
	*/
	...
	destTexture = this.webgl.createDestinationTexture(1, N);
	frameBuffer = this.webgl.bindDestinationTexture(1, N, destTexture);
	...
	rawbuffer = new ArrayBuffer(N*Float32Array.BYTES_PER_ELEMENT);

	// read the result into our buffer, as bytes
	prod = new Uint8Array(rawbuffer);
	gl.readPixels(0, 0, N, 1, gl.RGBA, gl.UNSIGNED_BYTE, prod);

...
/* create RGBA texture of width w/4 from given texels
   bind to texture unit and set as uniform in texture shader program to use

   alternative to textures?
   http://stackoverflow.com/questions/17203508/webgl-hardware-skinning-with-a-bone-texture
 */
WebGL.prototype.createInputTexture = function(h, w, texels){
	...
	*/
	if(pad == 0){
		...
		var full_row_texel_len = w - rem,
			full_row_texture_width = full_row_texel_len / COMPONENTS_PER_TEXEL;
		...
		for(var i = 0; i < h; i++){
			row_start = i * w;
			full_row_end = row_start + full_row_texel_len;
			row = new Float32Array(texels.buffer, row_start * texels.BYTES_PER_ELEMENT, full_row_texel_len);
			if(full_row_texel_len > 0){
			...
			remainder = new Float32Array(texels.buffer, full_row_end * texels.BYTES_PER_ELEMENT, rem);
 ...
 */
WebGL.prototype.bindDestinationTexture = function(M, N, texture) {
	var gl = this.context;

...
function WebGL(options) {
	...
	this.encode_program = this.createProgram(WebGL.ENCODE_FRAGMENT_SHADER);
};
...
void main(void) {                                                        \n\
	...
	float val = selectIndex(val_v, channel); \n\
	\n\
	if (val == 0.) {                                                     \n\

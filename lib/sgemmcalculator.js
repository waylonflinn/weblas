...
SGEMMCalculator.ROW_COUNT_UNIFORM_NAME = "M";
...
void main(void) {                                                        \n\
	...
	int channel = int(mod(col_t * float(M + pad), 4.0 )); \n\
...
SGEMMCalculator.prototype.calculate = function(M, N, K, alpha, A, B, beta, C, out){
	...
	var mod = (M % WebGL.COMPONENTS_PER_TEXEL),
		cpad = mod == 0 ? 0 : WebGL.COMPONENTS_PER_TEXEL - mod;
	...
	*/
	...
	this.bindUniforms(M, K, cpad, alpha, beta);
 ...
 */
SGEMMCalculator.prototype.bindUniforms = function(M, K, pad, alpha, beta) {
	var gl = this.webgl.context;
	...
	if(beta != null){
		...
		M_gl = gl.getUniformLocation(this.program, SGEMMCalculator.ROW_COUNT_UNIFORM_NAME);
		pad_gl = gl.getUniformLocation(this.program, SGEMMCalculator.PAD_UNIFORM_NAME);

...
GEMMFloatCalculator.TEXTURE_UNIFORM_NAME_2 = "Bt";
...
uniform sampler2D Bt;		// padded B transpose                        \n\
uniform int       K;		// interior (shared) dimension               \n\
uniform float     delta;	// space between items in shared dimension   \n\
																		 \n\
// sum row i x col j                                                     \n\
float sumrowcol(float i, float j) {                                      \n\
	...
	for (int l=0 ; l<4096 ; ++l) {                                       \n\
		...
		vec4 b_kj = texture2D(Bt, vec2(k, j));                           \n\
		sum += dot(a_ik, b_kj);                                          \n\
...
GEMMFloatCalculator.prototype.bindUniforms = function(K) {
	...
	var delta	= gl.getUniformLocation(this.program, "delta");

...
/* Linearize onto Kernels, Transform input into one row per patch, for use in
	...
	t0 - the input Tensor
 */
function slokn(channels, factor, stride, t0){
	...
	var N_p = Math.ceil((N - factor) / stride) + 1;
	var M_p = Math.ceil((M - factor) / stride) + 1;
	...
	slokncalculator.calculate(M, N, channels, M_out, N_out, N_p, factor, stride, t0.texture, tOut.texture);
